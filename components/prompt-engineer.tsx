...
const USE_CASES: {
...
}[] = [
	{
		...
		description: "A message or instruction you'd send to a chatbot",
	},
	{
		value: "agent-instructions",
		label: "Agent Instructions",
		description:
			"Instructions for an autonomous AI agent (e.g. CLAUDE.md, task briefs)",
	},
	{
		value: "system-prompt",
		label: "System Prompt",
		description: "Persistent behavior across a conversation",
	},
];

function splitPromptAndChangelog(text: string): {
	prompt: string;
...
export function PromptEngineer() {
	const [prompt, setPrompt] = usePersistedState("prompt-engineer-text", "");
	const [useCase, setUseCase] = usePersistedState<PromptUseCase>(
		"prompt-engineer-usecase",
		"chatbot-prompt",
	);
	...
	const { prompt: improvedPrompt, changelog } =
		splitPromptAndChangelog(streamedText);
	...
	const handleRegenerate = useCallback(() => {
		...
		if (feedback.trim()) {
			newEntries.push({
				question: "Additional feedback from user",
		...
		posthog.capture("prompt_regenerated", {
			use_case: useCase,
			round: nextRound,
			questions_answered: newEntries.filter(
				(e) => e.question !== "Additional feedback from user",
			).length,
	...
	const handlePromptChange = (value: string) => {
		setPrompt(value);
		if (state === "empty" && value.trim()) {
			setState("ready");
		} else if (state === "ready" && !value.trim()) {
			setState("empty");
		}
	};
	...
	return (
		<div className="flex flex-col gap-6">
			<div className="flex flex-col gap-3">
				...
				<div className="flex flex-col gap-1">
					<div className="flex items-center justify-between gap-3">
						<div className="flex items-center gap-3">
							<select
								...
								className={`h-11 rounded-md border border-input bg-background px-3 text-sm text-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring ${isLoading ? "pointer-events-none opacity-50" : ""}`}
								disabled={isLoading}
							>
								{USE_CASES.map((uc) => (
					...
					<p className="text-xs text-muted-foreground">
						{USE_CASES.find((uc) => uc.value === useCase)?.description}
					</p>
				</div>
			</div>
			...
			{(isLoading ||
				(state === "results" && (streamedText || thinkingText))) && (
										...
										<CollapsibleContent>
											<Card className="mt-2 py-0">
												<CardContent className="prose prose-sm max-w-none py-4 text-sm leading-relaxed dark:prose-invert [&_li]:mb-3 last:[&_li]:mb-0 [&>p+p]:mt-4">
													<ReactMarkdown>
														{changelog
															.replace(/^#{1,3}\s*What I changed\s*\n*/, "")
															.trimStart()}
													</ReactMarkdown>
												</CardContent>
											</Card>
