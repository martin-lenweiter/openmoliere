...
describe("parseClaudeResponse", () => {
	...
	it("handles non-standard language codes like N/A", () => {
		const response = "Some text.\n---LANGUAGE: N/A---\n---ERRORS_JSON---\n[]";
		const result = parseClaudeResponse(response);
		expect(result.correctedText).toBe("Some text.");
		expect(result.language).toBe("N/A");
	});

	it("handles language code 'Unknown'", () => {
		const response =
			"Some text.\n---LANGUAGE: Unknown---\n---ERRORS_JSON---\n[]";
		const result = parseClaudeResponse(response);
	...
	it("strips 'Corrected text:' preamble label", () => {
		const response = [
			"Corrected text:",
			"The fixed text here.",
			"---LANGUAGE: en-US---",
		...
		const result = parseClaudeResponse(response);
		expect(result.correctedText).toBe("The fixed text here.");
	});

	it("strips 'Here is the corrected version:' preamble label", () => {
		const response = [
	...
	it("handles missing fields in error objects with defaults", () => {
		const response = ["Text.", "---ERRORS_JSON---", JSON.stringify([{}])].join(
			"\n",
		);
		const result = parseClaudeResponse(response);
